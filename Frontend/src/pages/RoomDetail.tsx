...
import { PAYMENT_METHODS, paymentService, type PaymentMethod } from '../services/payment.service';
...
const RoomDetail = () => {
    ...
    const processBookingAndPayment = async () => {
        ...
        try {
            ...
            const newBookingId = (bookingData as any).data?.id || (bookingData as any).booking?.id || (bookingData as any).id;
            if (!newBookingId) throw new Error('Booking did not return an id');
    ...
    const imageUrl = room.images && room.images.length > 0
        ? room.images[currentImageIndex].imageUrl
        ...
        <div className="room-detail-page animate-fade-in">
                ...
                <div className="room-booking-card">
                    <div className="card sticky-card">
                        <div className="booking-price">
                            <h3>{formatCurrency(room.pricePerNight)}</h3> <span>/ night</span>
                        </div>
            ...
            <Modal isOpen={showPaymentModal} onClose={() => setShowPaymentModal(false)} title="Complete Your Booking">
                <div className="payment-modal-content">
                    <p><strong>Total Amount:</strong> {formatCurrency(calculateTotalPrice())}</p>
                    <div className="input-group" style={{ marginTop: '1rem' }}>
                        ...
                        <div className="payment-method-list">
                            {PAYMENT_METHODS.map(opt => (
                                <label
                                    key={opt.value}
                                    className={`payment-method-option ${paymentMethod === opt.value ? 'selected' : ''} ${opt.planned ? 'disabled' : ''}`}
                                >
                                    <input
                                        type="radio"
                                        name="paymentMethod"
                                        ...
                                        disabled={opt.planned}
                                        onChange={() => setPaymentMethod(opt.value)}
                                    />
                                    <div className="payment-method-text">
                                        <span className="payment-method-label">{opt.label}</span>
                                        {opt.planned && <span className="payment-method-tag">Future planned</span>}
                                    </div>
                                </label>
                            ))}
                        </div>
                        <p style={{ marginTop: '0.75rem', fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
                            Only Cash on Arrival is supported right now. Bank transfer and online card payments are coming soon.
                        </p>
                    </div>

                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', marginTop: '2rem' }}>
                        <button className="btn-text" onClick={() => setShowPaymentModal(false)} disabled={isProcessingPayment}>Cancel</button>
                        <button className="btn-primary" onClick={processBookingAndPayment} disabled={isProcessingPayment || paymentMethod !== 'CASH'}>
